...
public class LampeMagiqueMainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        plusRouge = findViewById(R.id.plusRouge);
        moinsRouge = findViewById(R.id.moinsRouge);
        plusVert = findViewById(R.id.plusVert);
        moinsVert = findViewById(R.id.moinsVert);
        plusBleu = findViewById(R.id.plusBleu);
        moinsBleu = findViewById(R.id.moinsBleu);
    ...
    public void onClick(View view) {
        ...
        if (view.getId() == R.id.couleur) {
            new ProgrammationCouleur().start();
        }
    }
    ...
    private class ProgrammationCouleur extends Thread {
        ...
        public void run() {
            ...
            changerInterface(rouge, vert, bleu);
        }

        private void calculCouleur(int count) {
            switch (count) {
                case 1:
                    v = 255;
                    break;
            ...
            changerInterface(r, v, b);
        }

        private void changerInterface(final int ro, final int ve, final int bl) {
            handler.post(new Runnable() {
                @Override
                public void run() {
                    changerCouleur(ro, ve, bl);
                }
            });
        }
    }
    ...
    private class CommunicationServeur extends Thread {

        private int ro, ve, bl;

        public CommunicationServeur(int ro, int ve, int bl) {
            this.ro = ro;
            this.ve = ve;
            this.bl = bl;
        }
        ...
        public void run() {
            try {
                ...
                String r = Integer.toHexString(ro);
                if (r.length() == 1) r = "0" + r;
                String v = Integer.toHexString(ve);
                if (v.length() == 1) v = "0" + v;
                String b = Integer.toHexString(bl);
                if (b.length() == 1) b = "0" + b;
    ...
    private void changerCouleur(int r, int v, int b) {
        ...
        new CommunicationServeur(r, v, b).start();
